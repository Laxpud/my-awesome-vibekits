...
- `plugins/laxpud-vibekits/`：Codex marketplace 可安装插件包，保留当前 Codex CLI 要求的 `./plugins/<plugin-name>` 布局
...
5. **适配层隔离**：平台特定配置必须放在对应平台的官方适配位置，例如 Claude Code 使用 `.claude-plugin/`，Codex 使用 `.codex-plugin/`、`plugins/laxpud-vibekits/` 和 `.agents/plugins/marketplace.json`，不得污染通用技能/规则内容
6. **Codex包同步**：更新根目录 `skills/` 后，必须同步更新 `plugins/laxpud-vibekits/skills/`，否则 GitHub marketplace 安装到的技能内容会落后于仓库源内容
